...
parser.add_argument('--base-lr', '--base-learning-rate', default=1e-6, type=float,
                    help='base learning rate (default: 1e-6)')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # ----------------------------------------------------------------------------------------
    Raman_Dataset_Train = dataset.RamanImageDataset(train_ids, os.path.dirname(args.dataset), args.labels, 
                                                    batch_size = args.batch_size, image_size = args.image_size, 
                                                    spectrum_len = args.spectrum_len, spectrum_shift = 0.1, spectrum_flip = True, 
                                                    horizontal_flip = True, vertical_flip = True, rotate = True, patch = True)

    Raman_Dataset_Val = dataset.RamanImageDataset(val_ids, os.path.dirname(args.dataset), args.labels, 
                                                  batch_size = args.batch_size, image_size = args.image_size, 
    ...
    train_loader = DataLoader(Raman_Dataset_Train, batch_size = args.batch_size, shuffle = True, 
    ...
    if args.rank == 0:
        args.network = 'ResUNet' if args.residual else args.network
    ...
    for epoch in range(args.epochs):
        train_loss = train(train_loader, net, optimizer, scheduler, criterion, epoch, args)
        valid_loss = validate(val_loader, net, criterion, args)
...
def train(dataloader, net, optimizer, scheduler, criterion, epoch, args):
...
def validate(dataloader, net, criterion, args):
